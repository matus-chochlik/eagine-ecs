...
void manager_component_register_1(auto& s) {
    ...
    eagine::ecs::basic_manager<eagine::identifier_value> mgr;
    ...
    mgr
      .register_component_storage<eagine::ecs::flat_map_cmp_storage, greeting>();
...
void manager_component_register_2(auto& s) {
    ...
    mgr.register_relation_storage<eagine::ecs::flat_map_rel_storage, mother>();

    test.check(mgr.knows_relation_type<mother>(), "mother 2");
    test.check(not mgr.knows_relation_type<father>(), "father 2");
    mgr.register_relation_storage<eagine::ecs::flat_map_rel_storage, father>();
...
void manager_component_add_has_name_1(auto& s) {
    ...
    mgr.add("john", greeting("Hi"));
    mgr.add("jane", person("Jane", "Doe"), greeting("Hi"));
    mgr.add("bill", person("Bill", "Roe")).add("bill", greeting("Hello"));

    test.check(
      mgr.ensure<person>("john").has_name("John", "Doe"), "has name 1");
    test.check(
      mgr.ensure<person>("jane").has_name("Jane", "Doe"), "has name 2");
    test.check(
      mgr.ensure<person>("bill").has_name("Bill", "Roe"), "has name 3");
}
//------------------------------------------------------------------------------
// add / remove
//------------------------------------------------------------------------------
void manager_component_add_remove_1(auto& s) {
    ...
    test.check(mgr.has<person>("john"), "john person");
    test.check(mgr.has<person>("jane"), "jane person");
    test.check(mgr.has<greeting>("jane"), "jane greeting");
    test.check(mgr.has<person>("bill"), "bill person");
    test.check(mgr.has<greeting>("bill"), "bill greeting");

    mgr.remove<person>("john");
    test.check(not mgr.has<person>("john"), "john not person");

    mgr.add("john", greeting("Hi"));
    test.check(mgr.has<greeting>("john"), "greeting john");

    mgr.remove<greeting>("jane");
    test.check(not mgr.has<greeting>("jane"), "jane not greeting");

    mgr.remove<greeting, person>("bill");
    test.check(not mgr.has<person>("bill"), "bill not person");
    test.check(not mgr.has<greeting>("bill"), "bill not greeting");

    test.check(mgr.has<greeting>("john"), "greeting john");
    mgr.remove<person, greeting>("john");
    test.check(not mgr.has<person>("john"), "john not person");
    test.check(not mgr.has<greeting>("john"), "john not greeting");
}
//------------------------------------------------------------------------------
// add / forget
//------------------------------------------------------------------------------
void manager_component_add_forget_1(auto& s) {
    ...
    mgr.forget("jane");
    test.check(not mgr.has<person>("jane"), "jane not person");
    test.check(not mgr.has<greeting>("jane"), "jane not greeting");
...
void manager_component_for_each_1(auto& s) {
    ...
    std::map<eagine::identifier_value, std::tuple<std::string, std::string>>
      names;
...
void manager_component_for_each_2(auto& s) {
    ...
    add("John", "John", "Doe", "Hi");
    add("Jane", "Jane", "Roe", "Hey");
    add("Jack", "Jack", "Daniels", "Howdy");
...
void manager_component_for_each_3(auto& s) {
    ...
    mgr.add("John", person("John", "Doe"));
    mgr.add("Jane", person("Jane", "Doe"), greeting("Hi"));
    mgr.add("Bill", person("Bill", "Roe"), greeting("Hello"));
    mgr.add("Jack", greeting("Howdy"));

    mgr.for_each_opt<const person, const greeting>(
      {eagine::construct_from,
       [&](
         eagine::identifier_value,
...
void manager_component_show_hide_1(auto& s) {
    ...
    mgr.add("john", person("John", "Doe"), greeting("Hi"));
    mgr.add("jane", person("Jane", "Doe"), greeting("Hello"));
    mgr.add("bill", person("Bill", "Roe"));

    test.check(not mgr.is_hidden<person>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<greeting>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<person>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<greeting>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<person>("bill"), "not hidden (bill)");
    test.check(not mgr.is_hidden<greeting>("bill"), "not hidden (bill)");

    test.check(
      not mgr.are_hidden<person, greeting>("john"), "are not hidden (john)");
    test.check(
      not mgr.are_hidden<greeting, person>("jane"), "are not hidden (jane)");
    test.check(not mgr.are_hidden<person>("bill"), "are not hidden (bill)");

    mgr.hide<person>("john");

    test.check(mgr.is_hidden<person>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<greeting>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<person>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<greeting>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<person>("bill"), "not hidden (bill)");
    ...
    mgr.hide<greeting>("jane");

    test.check(mgr.is_hidden<person>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<greeting>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<person>("jane"), "not hidden (jane)");
    test.check(mgr.is_hidden<greeting>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<person>("bill"), "not hidden (bill)");
    test.check(not mgr.is_hidden<greeting>("bill"), "not hidden (bill)");

    mgr.hide<person>("jane");

    test.check(mgr.is_hidden<person>("john"), "not hidden (john)");
    test.check(not mgr.is_hidden<greeting>("john"), "not hidden (john)");
    test.check(mgr.is_hidden<person>("jane"), "not hidden (jane)");
    test.check(mgr.is_hidden<greeting>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<person>("bill"), "not hidden (bill)");
    test.check(not mgr.is_hidden<greeting>("bill"), "not hidden (bill)");
    test.check(
      mgr.are_hidden<greeting, person>("jane"), "are not hidden (jane)");

    mgr.hide<greeting>("john");

    test.check(mgr.is_hidden<person>("john"), "not hidden (john)");
    test.check(mgr.is_hidden<greeting>("john"), "not hidden (john)");
    test.check(mgr.is_hidden<person>("jane"), "not hidden (jane)");
    test.check(mgr.is_hidden<greeting>("jane"), "not hidden (jane)");
    test.check(not mgr.is_hidden<person>("bill"), "not hidden (bill)");
    test.check(not mgr.is_hidden<greeting>("bill"), "not hidden (bill)");
    test.check(
      mgr.are_hidden<greeting, person>("john"), "are not hidden (john)");
    test.check(
      mgr.are_hidden<greeting, person>("jane"), "are not hidden (jane)");

    mgr.read_each<person>([&](eagine::identifier_value eid, auto&) {
        test.check(eid == id_v("bill"), "only bill");
        trck.checkpoint(1);
    });

    mgr.show<greeting, person>("jane");
    ...
    test.check(not mgr.is_hidden<greeting>("bill"), "not hidden (bill)");
    test.check(
      not mgr.are_hidden<greeting, person>("john"), "are not hidden (john)");
    test.check(
      not mgr.are_hidden<person, greeting>("jane"), "are not hidden (jane)");
}
//------------------------------------------------------------------------------
// relation / has
//------------------------------------------------------------------------------
